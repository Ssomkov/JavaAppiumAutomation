...
abstract public class ArticlePageObject extends MainPageObject {
    ...
    public String getValueArticleTitle(String articleTitle) {
        ...
        } else return this.waitForElementAndGetText(title, "Не удалось получить заголовок статьи: " + articleTitle, 10);
    }
    ...
    public void removeArticleFromSavedIfItAdded() {
        if (this.isElementPresent(OPTIONS_REMOVE_FROM_MY_LIST_BUTTON)) {
            this.waitForElementAndClick(OPTIONS_REMOVE_FROM_MY_LIST_BUTTON, "Не  удалось нажать кнопку удаления статьи из избранного", 1);
        }
        this.waitForElementPresent(OPTIONS_ADD_TO_MY_LIST_BUTTON, "Не найдена кнопка добавления статьи в избранное послее ее удаления из избранного");
    }
}
